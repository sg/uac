...
- UAC now collects copies of '/proc/<pid>/exe' and their related '/proc/<pid>/fd/*' if they are shown up as being deleted on Linux systems. Also, copies of '/proc/<pid>/object/a.out' will also be collected if they are shown as being deleted on Solaris systems. They are copied using 'dd conv=swab' tool in order to avoid UAC output file being flagged and quarantined by any antivirus tool.
...
- New artifact to collect the binary of (malicious) processes after they have been deleted (Linux and Solaris systems only) (live_response/process/deleted.yaml).
- New artifact to collect files of (malicious) processes after they have been deleted on (Linux systems only) (live_response/process/deleted.yaml).
